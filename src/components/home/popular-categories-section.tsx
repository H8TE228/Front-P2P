      ...
      <div className="mt-6 grid w-full grid-cols-2 gap-4 sm:grid-cols-4 lg:grid-cols-8">
        {CATEGORIES.map(({ id, name, icon: Icon }) => (
          <Link
            ...
            key={id}
            type="button"
            className="flex min-h-[114px] w-full min-w-0 cursor-pointer flex-col items-center gap-3 rounded-[14px] border border-[#E2E8F0] bg-white px-4 pt-4 pb-4 text-center transition-colors hover:bg-[#F8FAFC] dark:border-[#1D293D] dark:bg-[#0F172B] dark:hover:bg-[#0F172B]"
          >
            ...
            <span className="text-sm leading-5 font-medium tracking-normal text-[#314158] dark:text-[#CAD5E2]">
              {name}
            </span>
          </Link>
        ))}
